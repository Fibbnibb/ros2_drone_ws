...
class FormationDrone(Node):
    def __init__(self, namespace, is_leader=False, formation_distance=2.0):
        ...
        self.home_position.pose.position.z = 0.0
    ...
    def publish_setpoint(self):
        ...
        # If we're a follower, apply formation control to update the setpoint
    ...
    def apply_formation_control(self):
        """Apply simple formation control to maintain position relative to leader"""
        ...
        for ns, info in self.other_drones.items():
            if "drone1" in ns:  # Assuming drone1 is the leader
                ...
                max_speed = 1.5 # Maximum allowed speed in m/s
                
                # Scale speed based on distance
                speed_scale = min(distance / 2.0, 1.0)  # Slow down as we get closer
                velocity = to_target * speed_scale
                
                # Limit velocity magnitude
                vel_mag = np.linalg.norm(velocity)
                if vel_mag > max_speed:
                    velocity = velocity / vel_mag * max_speed
                ...
                # Safety: Don't go below minimum altitude
                self.setpoint.pose.position.z = max(self.setpoint.pose.position.z, 1.5)
                break
